...
/// Windows implementation of [WindowManagerPlatform].
///
/// Delegates every call to the Pigeon-generated [WindowHostApi], which routes
/// to the C++ side via `IcefelixWindowManagerWindowsPlugin` (registered by the
/// `IcefelixWindowManagerWindowsPluginCApi` C entry point). Native-to-Dart
/// callbacks are wired through [registerFlutterApi].
class IcefelixWindowManagerWindows extends WindowManagerPlatform {
  /// Creates the Windows platform implementation.
  IcefelixWindowManagerWindows() : super();

  /// Auto-registered by Flutter when the plugin loads (per pubspec.yaml
  /// flutter.plugin.platforms.windows.dartPluginClass).
  static void registerWith() {
    WindowManagerPlatform.instance = IcefelixWindowManagerWindows();
  }
